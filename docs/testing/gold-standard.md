...
The machine-readable audit supersedes hand-maintained supported-ID counts. The Phase 1 exit baseline contained 19 exact cases; Simple Past Transitive v0.1 raises the current result to 21 by activating two existing entity/relation cases. Full reproducible totals and per-case blockers are documented in the [Phase 1 completion audit](../architecture/phase-1-completion-audit.md).
