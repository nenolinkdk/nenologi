...
[Defeasible Recurrence Prediction Policy v0.1](../architecture/defeasible-recurrence-prediction-v0.1.md) makes `entailment_003` exact only in the full interpretation pipeline. Current metrics are therefore Phase 2 core 39/42 and full pipeline 40/42. Run the audit normally for Phase 2 or with `--full-pipeline` for the additive metric; `entailment_006` and `entailment_007` remain unimplemented.
