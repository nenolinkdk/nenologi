...
The machine-readable audit supersedes hand-maintained supported-ID counts. Successive controlled parser milestones raised the result from the 19-case Phase 1 exit baseline to 25. Condition/Omission Taxonomy Resolution v0.1 corrects the older `omission_001` expectation from proposition `OMISSION` to governing `CONDITION_CHANGE`, consistent with the explicit architecture, raising coverage to 26 exact with no analyzable-but-inexact cases. See the [taxonomy decision](../architecture/condition-omission-taxonomy-v0.1.md) and [Phase 1 completion audit](../architecture/phase-1-completion-audit.md).
