...
The machine-readable audit supersedes hand-maintained supported-ID counts. Successive milestones raised the result from the 19-case Phase 1 exit baseline to 28 exact cases. [Controlled Rule and Membership Representation v0.1](../architecture/rule-membership-representation-v0.1.md) does not change that total: it moves `entailment_002` internally from parser-unsupported representation to a complete normalized rule/membership analysis while leaving universal instantiation unimplemented. No inference gold expectation was changed.
