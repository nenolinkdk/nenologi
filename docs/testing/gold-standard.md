...
The machine-readable audit supersedes hand-maintained supported-ID counts. The Phase 1 exit baseline contained 19 exact cases; successive controlled parser milestones raised the result to 24, and Controlled Suffix-IF Conditions v0.1 raises it to 25 by activating `condition_002`. Gold expectations were unchanged. The same grammar makes `omission_001` analyzable but reveals its existing `OMISSION` expectation conflicts with condition precedence. Full results are documented in the [Phase 1 completion audit](../architecture/phase-1-completion-audit.md).
