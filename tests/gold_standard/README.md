...
Current coverage is 36 exact, 0 analyzable-but-inexact, 3 parser-unsupported, 0 comparator-unsupported, and 3 inference-not-implemented. Controlled embedded scope resolves `scope_001` and `scope_002`; every inference case remains parseable and represented. Probable prediction, alternative-reading evaluation, and non-literal interpretation policy remain unimplemented.
